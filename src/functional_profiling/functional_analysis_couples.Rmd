...
```{r}
#HUMAnN output file
humann_raw <- read_delim("data/functional_profiling/milk_infant_relab_pathabundance.tsv", delim = '\t')
#metadata table
...
metadata <- metadata %>% 
  select(Sample_ID, Sample_type, Bodysite, `Mother Infant Pair`)

merged.pathways.master <- left_join(hum_file.clean.long, metadata, by=c("Sample"="Sample_ID"))
```
