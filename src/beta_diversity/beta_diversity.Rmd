...
# Statistics on reads for milk and stool samples
...
my_metadata %>% 
  group_by(Bodysite) %>% 
  summarise(
    mean = mean(Reads_filtered, na.rm = T),
    sd = sd(Reads_filtered, na.rm = T)
  )
```
...
```{r}
#milk samples only
pdf(file=paste0("figures/filtered_reads_distribution_MilkOnly.pdf"), width=width_size, height=height_size)
ggplot(my_metadata %>% filter(Bodysite=="Milk"), aes(x=Reads_filtered, color=Sample_type, fill=Sample_type)) +
  geom_density(alpha=0.3) +
  theme_light() + 
  xlim(0, 18000000)+
  scale_color_manual(values=c("#e66101", "#fdb863")) +
...
## Stool samples only
```{r}
pdf(file=paste0("figures/filtered_reads_distribution_StoolOnly.pdf"), width=width_size, height=height_size)
ggplot(my_metadata %>% filter(Bodysite=="Stool"), aes(x=Reads_filtered, color=Sample_type, fill=Sample_type)) +
  geom_density(alpha=0.3) +
  theme_light() + 
  scale_color_manual(values=c("#5e3c99", "#b2abd2")) +
...
pdf(file=paste0("figures/PCoA_sample_type.pdf"), width=6, height=4)
...
pdf(file=paste0("figures/PCoA_mean_bifidos_relab_all.pdf"), width=4.5, height=3)
...
pdf(file=paste0("figures/PCoA_timepoint.pdf"), width=width_size, height=height_size)
...
pdf(file=paste0("figures/PCoA_prenatal_abx.pdf"), width=width_size, height=height_size)
prenatal.abx.plot
invisible(dev.off())
...
pdf(file=paste0("figures/PCoA_abx_any.pdf"), width=width_size, height=height_size)
abx.any.plot
invisible(dev.off())
...
pdf(file=paste0("figures/PCoA_delivery_mode.pdf"), width=5, height=height_size)
...
pdf(file=paste0("figures/PCoA_hospital.pdf"), width=6, height=height_size)
hospital.plot
invisible(dev.off())
...
pdf(file=paste0("figures/PCoA_exc_bf_6m.pdf"), width=width_size, height=height_size)
excl.bf.6m.plot
invisible(dev.off())
...
pdf(file=paste0("figures/cum_bifidos_relab.pdf"), width=8, height=10)
panel
dev.off()
...
pdf(file=paste0("figures/PCoA_biplot_species.pdf"), width=5.1, height=height_size)
pcoa.labels.plot
invisible(dev.off())
