...
## Load tables
```{r}
mpa_file <- read_delim("data/milk_infant_rel_abundance_contam_rm.txt", delim = '\t')
my_metadata <- read_delim("data/metadata_complete.csv", delim = ',')
bifidos_rel_abs_mean <- read_delim("data/predominance_group/bifidos_relabs_mean_all.csv", delim = ',')
bifidos_rel_abs <- read_delim("data/predominance_group/bifidos_relabs.csv", delim = ',')
```


## Calculate distances - PCoA with Bray-Curtis distance
```{r}
mpa_file = mpa_file[-1,]
# 
...
# 
# #order of samples in species_t_matrix needs to be the same fed into col_binds function when metadata are added!!
...
pdf(file=paste0("figures/beta_diversity/sample_type.pdf"), width=6, height=4)
...
## PCoA mean relative abundance bifidobacteria
```{r}
width_size=5
height_size=3
...
pdf(file=paste0("figures/beta_diversity/mean_bifidos_relab_all.pdf"), width=4.5, height=3)
...
## PCoA divided by collection time point in infant stool samples only
```{r}
...
pdf(file=paste0("figures/beta_diversity/timepoint.pdf"), width=width_size, height=height_size)
...
## PCoA stratified by pre-partum antibiotic intake
```{r}
...
pdf(file=paste0("figures/beta_diversity/prenatal_abx.pdf"), width=width_size, height=height_size)
prenatal.abx.plot
invisible(dev.off())
...
## PCoA stratified by general antibiotics intake 
```{r}
...
pdf(file=paste0("figures/beta_diversity/abx-any.pdf"), width=width_size, height=height_size)
abx.any.plot
invisible(dev.off())
...
## PCoA stratified by delivery mode
```{r}
...
pdf(file=paste0("figures/beta_diversity/delivery_mode.pdf"), width=5, height=height_size)
...
## PCoA stratified by place of birth
```{r}
...
pdf(file=paste0("figures/beta_diversity/hospital.pdf"), width=6, height=height_size)
hospital.plot
invisible(dev.off())
...
## PCoA stratified by feeding mode at 6 months of age 
```{r}
...
pdf(file=paste0("figures/beta_diversity/exc_bf_6m.pdf"), width=width_size, height=height_size)
excl.bf.6m.plot
invisible(dev.off())
